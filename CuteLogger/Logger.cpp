...
{
    if (Q_UNLIKELY(!LogGlobalSetup::self().consoleLoggingEnabled()))
    {
        return;
    }
    va_list va;
    va_start(va, msg);
    QString message = QString().asprintf("<%p> ", QThread::currentThreadId())
            + QString().vasprintf(msg, va);
    m_l->write(m_level, m_file, m_line, m_function, m_category, message);
    va_end(va);
}
...
void CuteMessageLogger::write(const QString& msg) const
{
    ...
    QString message = QString().asprintf("<%p> ", QThread::currentThreadId()) + msg;
    m_l->write(m_level, m_file, m_line, m_function, m_category, message);
}
