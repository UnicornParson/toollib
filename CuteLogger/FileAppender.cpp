...
// STL
#include <iostream>
#define FILE_SIZE_LIMIT_DISABLED -1
#define DEFAULT_FILE_SIZE_LIMIT FILE_SIZE_LIMIT_DISABLED
/**
 * \class FileAppender
 *
...
FileAppender::FileAppender(const QString& fileName):
    m_sizeLimit(DEFAULT_FILE_SIZE_LIMIT),
    m_currentFileIndex(0)
{
    setFileName(fileName);
    setFormat("[%{type:-7}] <%{function}.%{line}> %{message}\n");
}
...
void FileAppender::checkFileSizeLimit()
{
    if (m_sizeLimit > 0 && m_logFile.size() >= (m_sizeLimit*1024*1024))
    {
        m_logFile.close();
        m_currentFileIndex++;
        QString newFileName = QString("%1.%2").arg(m_logFileName).arg(m_currentFileIndex);
        m_logFile.setFileName(newFileName);
        openFile();
    }
}

void FileAppender::setLogFileSizeLimitMB(int sizeLimit)
{
