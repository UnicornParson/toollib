                                         ...
                                         QAESEncryption::Padding padding)
{
    ...
    case Padding::ISO:
    {
        ...
        if (ret.at(marker_index) == char(0x80))
        {
...
void QAESEncryption::mixColumns()
{
    ...
    for (int i = 0; i < 16; i += 4){
        ...
        tmp     =  static_cast<quint8>(it[i])
                ^ static_cast<quint8>(it[i+1])
                ^ static_cast<quint8>(it[i+2])
                ^ static_cast<quint8>(it[i+3]);

        tm      = xTime(static_cast<quint8>(it[i]) ^ static_cast<quint8>(it[i+1]));

        it[i]   = static_cast<quint8>(it[i])
                ^ static_cast<quint8>(tm)
                ^ static_cast<quint8>(tmp);

        tm      = xTime(static_cast<quint8>(it[i+1]) ^ static_cast<quint8>(it[i+2]));
        it[i+1] = static_cast<quint8>(it[i+1])
                ^ static_cast<quint8>(tm)
                ^ static_cast<quint8>(tmp);

        tm      = xTime(static_cast<quint8>(it[i+2]) ^ static_cast<quint8>(it[i+3]));

        it[i+2] = static_cast<quint8>(it[i+2])
                ^ static_cast<quint8>(tm)
                ^ static_cast<quint8>(tmp);

        tm      = xTime(static_cast<quint8>(it[i+3]) ^ static_cast<quint8>(t));
        it[i+3] = static_cast<quint8>(it[i+3])
                ^ static_cast<quint8>(tm)
                ^ static_cast<quint8>(tmp);
    }
}
...
void QAESEncryption::invShiftRows()
{
    ...
    //Shift 3
    temp   = static_cast<quint8>(it[15]);
    it[15] = static_cast<quint8>(it[3]);
    it[3]  = static_cast<quint8>(it[7]);
    it[7]  = static_cast<quint8>(it[11]);
    it[11] = static_cast<quint8>(temp);
}
...
QByteArray QAESEncryption::removePadding(const QByteArray &rawText)
{
    return RemovePadding(rawText, static_cast<Padding>(m_padding));
}
#ifdef _MSC_VER
#pragma warning( pop )
#else
